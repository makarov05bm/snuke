...
Detects standard S3 buckets and static website hosting endpoints.\b
    - **Path-style:** `s3.amazonaws.com/bucket-name`\b
    - **Virtual-hosted style:** `bucket-name.s3.amazonaws.com`\b
    - **Regional endpoints:** `bucket-name.s3.us-west-2.amazonaws.com`\b
    - **S3 Websites:** `bucket-name.s3-website.us-east-1.amazonaws.com`

## 🔵 Azure Blob Storage
Detects various Azure Storage services (Blob, Data Lake Gen2, File, Queue, and Table).\b
    - **Blob Storage:** `accountname.blob.core.windows.net`\b
    - **Data Lake (DFS)**: `accountname.dfs.core.windows.net`\b
    - **File/Queue/Table:** Supports .file, .queue, and .table endpoints.

## 🟢 GCP (Google Cloud Platform)
Detects Google Cloud Storage (GCS) buckets using both the standard API and authenticated browser URLs.\b
    - **Standard API:** `storage.googleapis.com/bucket-name`\b
    - **Subdomain style:** `bucket-name.storage.googleapis.com`\b
